    ...
    this.setState(oldState, () => {
      if (
        ...
        var diffDays = parseInt(
          (this.state.toDate - this.state.fromDate) / (1000 * 60 * 60 * 24)
        );
          ...
          <button
            ...
            onClick={() => {
              ...
              this.props.onRent(
                ...
                this.state.toDate,
                this.state.diffDate,
                this.state.diffDate * this.props.pricePerDay +
                  this.props.deposit
              );
              this.props.history.replace(
                "/payment/" + this.props.match.params.id
              );
            }}
          >
            Rent!
        ...
        <div className={classes.Div}>
          ...
          <Form validated={this.state.validated} onSubmit={this.rentHandler}>
            <FormGroup>
                ...
                <Col>
                  {this.state.diffDate ? (
                    <div style={{ textAlign: "center" }}>
                      <strong>
                        {this.state.diffDate} days Total Price:{" "}
                        {this.state.diffDate * this.props.pricePerDay +
                          this.props.deposit}
                        บาท
                      </strong>
              ...
              <Row>
                <Col sm={6}>{pickdate}</Col>
                <Col sm={6}>{returndate}</Col>
              </Row>
              {/* <hr />
                            <Row>
...
const mapDispatchToProps = dispatch => {
  return {
    onRent: (from, to, du, price) =>
      dispatch({
        type: actionTypes.SET_RENT,
        fromDate: from,
        toDate: to,
        duration: du,
        price: price
