...
const carPic = props => {
  const imagesPath = props.imagesPath.map((path, index) => (
    <Carousel.Item key={index}>
      <img className="d-block w-100 h-100" src={path} alt="carPic" />
    </Carousel.Item>
  ));
  return <Carousel>{imagesPath}</Carousel>;
};
...
const carDetailMiddle = props => {
  ...
  return (
    <div className={classes.Detail}>
      <p className={classes.buttomline} />
      <Row>
        <Col xs="3">YEAR </Col>
        <Col>: {props.payload.regYear}</Col>
      </Row>
      <Row>
        <Col xs="3">CAR NUMBER </Col>
        <Col>: {props.payload.LNumber}</Col>
      </Row>
      <Row>
        <Col xs="3">GEAR TYPE </Col>
        <Col>: {props.payload.gear === "auto" ? "Auto" : "Manual"}</Col>
      </Row>
      <Row>
        <Col xs="3">SEAT </Col>
        <Col>: {props.payload.seat}</Col>
      </Row>
      <Row>
        <Col xs="3">EQUIPMENT </Col>
        <Col>: {props.payload.equipment}</Col>
      </Row>
      <Row>
        <Col xs="3">DETAIL :</Col>
        <Col>
          ...
          <TextTruncate
            line={9}
            truncateText="..."
            text={props.payload.description}
          />
        </Col>
      </Row>
      <Row>
        <Col xs="3">PRICE PER DAY </Col>
      ...
      <Row>
        ...
        <Col>: {readableDateFrom} </Col>
      </Row>
      <Row>
        <Col xs="3">AVAILABLE UNTIL </Col>
        <Col>: {readableDateTo} </Col>
      </Row>
      <p className={classes.buttomline} />
      <Row>
        <Rent
